...
def all_operations(config, bucket_obj):
    """
    Run this function to do all the operations at once
    :parameters:
        config - object from configuration.py, contains info from config file
        bucket_obj - object that contains aws's session and bucket info
    """
    try:
        files = bucket_obj.get_files_list(config.source_dir)
        if not files:
        ...
        for current_file in files:
            try:
                if current_file.key.endswith(config.input_ext):
                    ...
                    # Read file from aws
                    data = bucket_obj.read_file(current_file.key)
                    ...
                    bucket_obj.write_output(out_json, output_key)

                    # Move file to archive
                    move_key = current_file.key.replace(config.source_dir, config.archive_dir)
                    bucket_obj.move_file(current_file.key, move_key)

                    # Delete file from source
                    bucket_obj.delete_file(current_file.key)
...
if __name__ == '__main__':
    ...
    bucket_object = conn_object.get_bucket_obj(config.bucket)
    bucket_obj = Aws(bucket_object)
    all_operations(config, bucket_obj)
